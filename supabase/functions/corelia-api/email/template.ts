import { resolveAppUrl, resolveEmailAssetBaseUrl } from "../lib/mail/layout.ts";
...
export function renderEmailDocument(params: EmailDocumentParams): { subject: string; html: string } {
  return render(params, { appUrl: resolveAppUrl(), assetBaseUrl: resolveEmailAssetBaseUrl() }, `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
}
