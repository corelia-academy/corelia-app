-- Reminder candidates must have an available curriculum. Keep cadence, opt-out,
-- historical completion and service-only wrapper/ACLs unchanged.
CREATE OR REPLACE FUNCTION private.get_learning_reminder_candidates()
RETURNS TABLE (
  user_id uuid,
  email text,
  full_name text,
  ...
  WITH user_inactivities AS (
    ...
    WHERE e.completed_at IS NULL
      ...
      AND EXISTS (
        ...
        WHERE lesson.course_id = c.id
          AND lesson.published AND lesson.archived_at IS NULL
      )
    GROUP BY e.user_id
  ),
  evaluated_users AS (
