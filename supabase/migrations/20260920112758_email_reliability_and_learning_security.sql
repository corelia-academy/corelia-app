...
-- The exposed RPC is an invoker wrapper. The privileged implementation stays
-- private and derives the target user from auth.uid(), so callers cannot choose
-- another learner.
CREATE OR REPLACE FUNCTION private.learning_revert_completion_for_current_user(
  p_course text,
  p_mode text DEFAULT 'last_lesson'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  RETURN private.learning_revert_completion(p_course, v_user, p_mode);
END;
$$;

CREATE OR REPLACE FUNCTION public.learning_revert_completion(
  p_course_id text,
  p_mode text DEFAULT 'last_lesson'
)
RETURNS jsonb
...
SET search_path = ''
AS $$
  SELECT private.learning_revert_completion_for_current_user(p_course_id, p_mode);
$$;

REVOKE ALL ON FUNCTION private.learning_revert_completion_for_current_user(text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.learning_revert_completion_for_current_user(text,text) TO authenticated;
