-- Scope learning analytics to the course as well as the reusable lesson ID.
-- Preserve existing course-feature permissions and private event payloads.
CREATE OR REPLACE FUNCTION private.learning_event(p_course text,p_lesson text,p_event text,p_passed boolean DEFAULT NULL) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
BEGIN
 ...
 IF p_event='lesson_started' AND EXISTS(SELECT 1 FROM public.activity_events WHERE actor_id=auth.uid() AND verb='learning.lesson_started' AND object_id=p_lesson AND target_type='course' AND target_id=p_course) THEN RETURN; END IF;
...
BEGIN
 ...
 'lessons',COALESCE((SELECT jsonb_agg(jsonb_build_object('id',l.id,'title',l.data->>'title','format',COALESCE(l.data->>'lesson_format','article'),
