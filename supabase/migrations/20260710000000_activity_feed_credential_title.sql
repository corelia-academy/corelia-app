-- Fix "earned credential credential" bug for Milestones
-- Milestones do not have a course_id or hackathon_id, so the activity feed payload
-- was missing a title, causing the UI to fall back to the generic noun "credential".

CREATE OR REPLACE FUNCTION private.credential_template_activity_payload(p_template_id uuid, p_payload jsonb DEFAULT '{}'::jsonb)
RETURNS jsonb
LANGUAGE sql
STABLE
AS $$
...
-- Backfill existing milestone credential events
