-- Validate resources for every published lesson, including previously saved locales.
CREATE FUNCTION private.learning_resource_errors(resources jsonb) RETURNS text[]
...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(resources) LOOP
   ...
   END IF;
   IF jsonb_typeof(item->'url') IS DISTINCT FROM 'string' OR COALESCE(item->>'url','') !~ '^https?://[^/@?#[:space:]]+([/?#][^[:space:]]*)?$' THEN
     errors:=array_append(errors,'resource_url_invalid');
   END IF;
 END LOOP;
 RETURN errors;
END $$;
REVOKE ALL ON FUNCTION private.learning_resource_errors(jsonb) FROM PUBLIC;
