-- Master Wave Forward Migration: Harden Course Enrollment Security Guard & Payment Provenance
-- Forward-only, data-preserving migration closing F-01:
-- 1. Prevent client-side forgery of paid_order_id and paid_at on public.enrollments
-- 2. Strictly verify payment_transactions ownership, course alignment, and 'paid' status
-- 3. Verify public.course_payment_access active status
-- 4. Preserve free course enrollment, staff bypass, and service_role settlement
...
BEGIN
  ...
  -- Check 2: If client provided paid_order_id, verify that it matches a REAL, PAID transaction for this user & course
