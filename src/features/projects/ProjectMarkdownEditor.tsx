...
export function ProjectMarkdownEditor({ label, value, onChange, maxLength, required, placeholder, rows = 10 }: {
  ...
  return <div className="min-w-0">
    <label htmlFor={id} className="text-sm font-medium">{label}{required ? <span className="text-primary"> *</span> : null}</label>
    <Tabs.Root value={mode} onValueChange={value => setMode(String(value))} className="mt-2 overflow-hidden rounded-lg border border-border bg-background">
      <Tabs.List aria-label={`${label} — Markdown`} className="flex gap-1 border-b border-border p-2">
        {(["write", "preview"] as const).map(tab => <Tabs.Tab key={tab} value={tab} className="min-h-10 rounded-md px-3 text-sm data-[active]:bg-primary/10 data-[active]:text-primary">{t(`projects.editor.${tab}`)}</Tabs.Tab>)}
      </Tabs.List>
    ...
    <p id={`${id}-limit`} className="mt-1 text-right text-xs text-foreground-muted">{t("projects.editor.characterLimit", { count: value.length, limit: maxLength })}</p>
  </div>;
}
