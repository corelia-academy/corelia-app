import { CODE_LANGUAGES, codeLanguages, codeFileForLanguage, type CodeLanguage } from "../languages";
...
export function CodeExerciseBuilder({ config, onChange }: { config: CodeExerciseConfig; onChange(config: CodeExerciseConfig): void }) {
  ...
  return <div id="learning-code_exercise_config" tabIndex={-1} className="space-y-5">
    ...
    <label className="block text-sm">{t("learning.mode")}<select className={inputClass} value={config.mode} onChange={async e => { const mode = e.target.value as "fill"|"edit"; if (await confirm(t("learning.dirtyConfirm"))) onChange(defaultCodeConfig(mode, config.language)); }}><option value="fill">{t("learning.fill")}</option><option value="edit">{t("learning.editCode")}</option></select></label>
    <label className="block text-sm">{t("learning.codeLanguage")}<select id="learning-code-language" className={inputClass} value={config.language} onChange={e => { const language = e.target.value as CodeLanguage; patch({ language, file: { ...config.file, path: codeFileForLanguage(config.file.path, language) } }); }}>{CODE_LANGUAGES.map(language => <option key={language} value={language}>{codeLanguages[language].label}</option>)}</select></label>
