...
import { changedLessonLocales, translationVideoIssues } from "../translationDraft";
...
export function LessonEditor(props: Props) {
  ...
  // Freeze both the draft and its saved baseline for this editing session.
  if(openedQuestions===null&&query.isFetchedAfterMount&&query.isSuccess)setOpenedQuestions(query.data);
  if(!props.isNew && getLessonFormat(props.initial)==="quiz" && openedQuestions===null)return <Dialog open onOpenChange={open=>!open&&props.onClose()}><DialogContent><DialogTitle>{t("learning.edit")}</DialogTitle><p>{t(query.isError?"learning.loadError":"learning.loading")}</p>{query.isError&&<Button onClick={()=>void query.refetch()}>{t("learning.retry")}</Button>}</DialogContent></Dialog>;
  return <Editor {...props} initialQuestions={openedQuestions??[]} />;
}
function Editor({courseId,finalAssignment,initial,sections,nextLessonOrder,primaryLocale,initialLocale,locales:initialLocales,isNew,initialIssueCodes,initialIssues,onDirtyChange,onClose,onSaved,initialQuestions}:Props&{initialQuestions:SectionQuestion[]}) {
  ...
  const [locale,setLocale]=useState(initialIssues?.[0]?.locale??initialLocale??primaryLocale);
  ...
  const mutation=useMutation({mutationFn:async()=>{const saved=await saveLearningLesson(courseId,lesson,lesson.lesson_format==="quiz"&&(isNew||lesson.id!==initial.id||JSON.stringify(questions)!==JSON.stringify(initialQuestions))?questions:undefined,changedLessonLocales(locales,lesson.id===initial.id?initialLocales:{}));return saved;},onSuccess:async()=>{try{if(lesson.lesson_format==="quiz")await invalidateLearningQuestions(client,courseId);await onSaved();}catch{toast.error(t("learning.savedRefreshFailed"));}onClose();}});
  ...
  const previewDisplay={...sourceDisplay,...normalizeLessonCopy(sourceDisplay).value};
  const display=master?{...lesson,...textCopy.value}:{...lesson,title:"",short_description:"",description_markdown:"",resources:[],youtube_url:"",youtube_start_seconds:0,youtube_end_seconds:null,...localized,...textCopy.value};
  const [ownVideoLocales,setOwnVideoLocales]=useState<Partial<Record<SupportedCourseLocale,boolean>>>({});
  const ownVideo=ownVideoLocales[locale]??Boolean(typeof localized.youtube_url==="string"&&localized.youtube_url.trim());
  async function save(){
    setError(null);
    ...
    nextIssues.push(...translationVideoIssues(lesson,locales,primaryLocale));
  ...
  const close=async()=>{if(mutation.isPending)return;if(!dirty||await confirm(t("learning.dirtyConfirm")))onClose();};
  const duplicate=()=>{
  ...
  return <>{confirmation}<Dialog open onOpenChange={open=>!open&&close()}><DialogContent initialFocus={() => issues[0] ? issueControl(issueField(issues[0],initial)) ?? true : true} className="w-full max-w-5xl"><DialogTitle>{isNew?t("learning.addLesson"):`${t("learning.edit")} · ${lesson.title}`}</DialogTitle><DialogDescription>{t("learning.reviewCourseOnly")}</DialogDescription>
    ...
    {preview?<><p>{t("learning.unsavedPreview")}</p><LessonPlayerCard lesson={previewDisplay} lessonIndex={0} courseId={courseId} mode="preview" contentLocale={locale} completed={false} isDraftLesson={false} hasFullCourseAccess previousLesson={null} nextLesson={null} translate={key=>String(t(key))} onMarkComplete={async()=>{}} onNavigateToLesson={()=>{}} questions={lesson.lesson_format==="quiz"?previewQuestions:undefined}/></>:<div className="space-y-5">
      ...
      {!master&&Object.keys(localized).length===0&&<p className="text-sm text-foreground-muted">{t("learning.translationMissing")}</p>}
      {!master&&<TranslationReference values={[lesson.title,lesson.short_description,lesson.description_markdown,...(Array.isArray(lesson.resources)?lesson.resources:[]).flatMap(r=>[r?.title,r?.url])]} />}
      ...
      {lesson.lesson_format==="video"&&<>{!master&&<><Toggle label={t("learning.useSourceVideo")} checked={!ownVideo} onChange={useSource=>{setOwnVideoLocales(previous=>({...previous,[locale]:!useSource}));if(useSource)localPatch({youtube_url:"",youtube_start_seconds:0,youtube_end_seconds:null});}}/>{!ownVideo&&<p className="text-sm">{lesson.youtube_url??t("learning.videoUpdating")}</p>}</>}{(master||ownVideo)&&<><TextField id="learning-youtube_url" label={t("learning.youtubeUrl")} value={display.youtube_url??""} onChange={youtube_url=>master?patch({youtube_url}):localPatch({youtube_url})}/><TextField label={t("learning.startSeconds")} type="number" value={String(display.youtube_start_seconds??0)} onChange={v=>master?patch({youtube_start_seconds:Number(v)}):localPatch({youtube_start_seconds:Number(v)})}/><TextField id="learning-youtube_end_seconds" label={t("learning.endSeconds")} type="number" value={display.youtube_end_seconds==null?"":String(display.youtube_end_seconds)} onChange={v=>master?patch({youtube_end_seconds:v?Number(v):null}):localPatch({youtube_end_seconds:v?Number(v):null})}/></>}</>}
      {lesson.lesson_format==="quiz"&&(invalidQuestions||invalidQuiz?<div id={invalidQuiz?"learning-quiz_config":"learning-questions"} tabIndex={-1}><p role="alert">{t("learning.invalidQuestionsRecovery")}</p><Button type="button" onClick={async()=>{if(!await confirm(t("learning.recoverQuestionsConfirm")))return;setQuestions(displayQuestions);patch({published:false,...(invalidQuiz?{quiz_config:{passing_ratio:typeof lesson.quiz_config?.passing_ratio==="number"?lesson.quiz_config.passing_ratio:0.7,allow_retry:typeof lesson.quiz_config?.allow_retry==="boolean"?lesson.quiz_config.allow_retry:true}}:{})});setLocale(primaryLocale);}}>{t("learning.recoverQuestions")}</Button></div>:master?<><TextField id="learning-quiz_config" label={t("learning.passingScore")} type="number" value={String((lesson.quiz_config?.passing_ratio??0.7)*100)} onChange={v=>patch({quiz_config:{allow_retry:lesson.quiz_config?.allow_retry??true,passing_ratio:Number(v)/100}})}/><QuizBuilder questions={questions} onChange={setQuestions} courseId={courseId} lessonId={lesson.id}/></>:questions.map(q=><div key={q.id} className="space-y-3 rounded-xl border border-border p-4"><TranslationReference values={[q.question,...q.options.map(o=>o.text),q.explanation]}/><TextField label={t("learning.question")} value={questionCopy?.[q.id]?.question??""} onChange={question=>localPatch({question_copy:{...localized.question_copy,[q.id]:{...questionCopy?.[q.id],question}}})}/>{q.options.map(o=><TextField key={o.id} label={t("learning.option")} value={questionCopy?.[q.id]?.options?.[o.id]??""} onChange={value=>localPatch({question_copy:{...localized.question_copy,[q.id]:{...questionCopy?.[q.id],options:{...questionCopy?.[q.id]?.options,[o.id]:value}}}})}/>)}<TextField label={t("learning.explanation")} value={questionCopy?.[q.id]?.explanation??""} onChange={explanation=>localPatch({question_copy:{...localized.question_copy,[q.id]:{...questionCopy?.[q.id],explanation}}})}/></div>))}
      {lesson.lesson_format==="practice"&&(invalidPractice?<div id="learning-practice_config" tabIndex={-1} className="space-y-3 rounded-lg border border-destructive/30 p-3"><p role="alert">{t("learning.invalidPracticeRecovery")}</p>{master?<Button type="button" variant="outline" onClick={async()=>{if(await confirm(t("learning.resetPracticeConfirm"))){patch({practice_config:{mode:"instruction"},published:false});setIssues([]);setError(null);}}}>{t("learning.resetPracticeConfig")}</Button>:<Button type="button" variant="outline" onClick={()=>setLocale(primaryLocale)}>{t("learning.editMasterConfig")}</Button>}</div>:master?<PracticeBuilder config={lesson.practice_config??{mode:"instruction"}} onChange={practice_config=>patch({practice_config})}/>:<><TranslationReference values={[...(lesson.practice_config?.checklist_items??[]).map(item=>item.label),...(lesson.practice_config?.project_steps??[]).flatMap(step=>[step.title,step.instructions_markdown])]}/>{lesson.practice_config?.checklist_items?.map(item=><TextField key={item.id} label={item.label} value={localized.practice_copy?.[item.id]?.label??""} onChange={label=>localPatch({practice_copy:{...localized.practice_copy,[item.id]:{label}}})}/>)}{lesson.practice_config?.project_steps?.map(step=><div key={step.id}><TextField label={t("learning.title")} value={localized.practice_copy?.[step.id]?.title??""} onChange={title=>localPatch({practice_copy:{...localized.practice_copy,[step.id]:{...localized.practice_copy?.[step.id],title}}})}/><TextField label={t("learning.instructions")} multiline value={localized.practice_copy?.[step.id]?.instructions_markdown??""} onChange={instructions_markdown=>localPatch({practice_copy:{...localized.practice_copy,[step.id]:{...localized.practice_copy?.[step.id],instructions_markdown}}})}/></div>)}</>)}
      {lesson.lesson_format==="code_exercise"&&(invalidCode?<div id="learning-code_exercise_config" tabIndex={-1} className="space-y-3 rounded-lg border border-destructive/30 p-3"><p role="alert">{t("learning.invalidCodeRecovery")}</p>{master?<Button type="button" variant="outline" onClick={async()=>{if(await confirm(t("learning.resetCodeConfirm"))){patch({code_exercise_config:defaultCodeConfig(),published:false});setIssues([]);setError(null);}}}>{t("learning.resetCodeConfig")}</Button>:<Button type="button" variant="outline" onClick={()=>setLocale(primaryLocale)}>{t("learning.editMasterConfig")}</Button>}</div>:lesson.code_exercise_config&&(master?<CodeExerciseBuilder config={lesson.code_exercise_config} onChange={code_exercise_config=>patch({code_exercise_config})}/>:invalidCopy?null:<><TranslationReference values={[...(lesson.code_exercise_config.hints??[]),...(lesson.code_exercise_config.blanks??[]).map(b=>b.feedback),...(lesson.code_exercise_config.tests??[]).flatMap(test=>[test.description,test.failure_message,test.hint])]}/><TextField label={t("learning.hints")} multiline value={(localized.code_exercise_locale?.hints??[]).join("\n")} onChange={value=>localPatch({code_exercise_locale:{...localized.code_exercise_locale,hints:value.split("\n")}})}/>{lesson.code_exercise_config.blanks?.map(b=><TextField key={b.id} label={`${t("learning.feedback")} · ${b.id}`} value={localized.code_exercise_locale?.blank_feedback?.[b.id]??""} onChange={value=>localPatch({code_exercise_locale:{...localized.code_exercise_locale,blank_feedback:{...localized.code_exercise_locale?.blank_feedback,[b.id]:value}}})}/>)}{lesson.code_exercise_config.tests?.map(test=><div key={test.id}><TextField id={`learning-test-${test.id}-description`} label={t("learning.description")} value={localized.code_exercise_locale?.test_copy?.[test.id]?.description??""} onChange={description=>localPatch({code_exercise_locale:{...localized.code_exercise_locale,test_copy:{...localized.code_exercise_locale?.test_copy,[test.id]:{...localized.code_exercise_locale?.test_copy?.[test.id],description}}}})}/><TextField label={t("learning.feedback")} value={localized.code_exercise_locale?.test_copy?.[test.id]?.failure_message??""} onChange={failure_message=>localPatch({code_exercise_locale:{...localized.code_exercise_locale,test_copy:{...localized.code_exercise_locale?.test_copy,[test.id]:{...localized.code_exercise_locale?.test_copy?.[test.id],failure_message}}}})}/><TextField label={t("learning.hints")} value={localized.code_exercise_locale?.test_copy?.[test.id]?.hint??""} onChange={hint=>localPatch({code_exercise_locale:{...localized.code_exercise_locale,test_copy:{...localized.code_exercise_locale?.test_copy,[test.id]:{...localized.code_exercise_locale?.test_copy?.[test.id],hint}}}})}/></div>)}</>))}
