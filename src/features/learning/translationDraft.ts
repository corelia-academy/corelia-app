...
export function changedLessonLocales(value: LessonLocales, saved: LessonLocales): LessonLocales {
...
export function translationVideoIssues(lesson: CourseLesson, locales: LessonLocales, primaryLocale: SupportedCourseLocale) {
  return Object.entries(locales).flatMap(([locale, copy]) => {
    if (locale === primaryLocale || typeof copy.youtube_url !== "string" || !copy.youtube_url.trim()) return [];
