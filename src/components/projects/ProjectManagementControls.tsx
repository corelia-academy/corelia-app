  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!mutation.isPending) setOpen(next); }}>
      <DialogContent className="sm:max-w-lg" showCloseButton={!mutation.isPending}>
        ...
        {admin ? <label className="grid gap-2 text-sm">{t("projects.management.action")}
        ...
        {admin || profile?.role === "admin" ? <label className="grid gap-2 text-sm">{t("projects.management.reason")}
          <textarea required maxLength={1000} rows={3} value={reason} disabled={mutation.isPending} onChange={e => setReason(e.target.value)} className="rounded-md border border-border bg-surface-base p-3" />
          <span className="text-xs text-foreground-muted">{reason.length} / 1000</span>
        </label> : null}
