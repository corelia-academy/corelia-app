...
export function ProjectCard({ project, ownerLabel, ownerHandle, taxonomy, hearted, className }: ProjectCardProps) {
  ...
  return <article className={cn("group relative flex h-full min-w-0 flex-col rounded-2xl border border-border-subtle bg-surface-base p-5 shadow-card transition-shadow hover:border-primary/40 hover:shadow-md",className)}>
    ...
    <h2 className="mt-4 line-clamp-2 break-words text-heading-small font-display text-foreground"><NavLink to={detailPath} className="rounded-sm hover:text-primary focus-visible:outline-2 focus-visible:outline-primary">{project.title}</NavLink></h2>
    <p className="mt-2 min-h-16 line-clamp-3 text-body-medium font-body text-foreground-muted">{project.summary || t("projects.card.noSummary")}</p>
    <dl className="mt-5 space-y-3 text-body-small font-body">
      {technologies.length ? <div className="flex items-start gap-3"><dt className="w-20 shrink-0 text-label-small font-body text-foreground-subtle">{t("projects.filters.techStacks")}</dt><dd className="min-w-0 font-medium">{technologies.map(item => item.name).join(", ")}</dd></div> : null}
      <div className="flex items-center gap-3"><dt className="w-20 shrink-0 text-label-small font-body text-foreground-subtle">{t("projects.editor.builder")}</dt><dd className="min-w-0 truncate font-medium">{ownerHandle ? <NavLink to={`/@${ownerHandle}`} className="hover:underline">{ownerLabel || `@${ownerHandle}`}</NavLink> : ownerLabel || t("projects.editor.builder")}</dd></div>
    </dl>
    {awards.length ? <div className="mt-4 flex flex-wrap gap-1.5"><span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-1 text-label-small font-body text-primary"><Trophy className="size-3" />{t("projects.editor.winner")}</span></div> : null}
