...
export function ProjectCard({ project, ownerLabel, ownerHandle, taxonomy, hearted, className }: ProjectCardProps) {
  ...
  return <article className={cn("group relative flex h-full min-w-0 flex-col rounded-2xl border border-border-subtle bg-surface-base p-5 shadow-card transition-shadow hover:border-primary/40 hover:shadow-md",className)}>
    ...
    <h2 className="mt-4 line-clamp-2 break-words text-lg font-semibold leading-snug"><NavLink to={detailPath} className="rounded-sm hover:text-primary focus-visible:outline-2 focus-visible:outline-primary">{project.title}</NavLink></h2>
    <p className="mt-2 min-h-16 line-clamp-3 text-sm leading-6 text-foreground-muted">{project.summary || t("projects.card.noSummary")}</p>
