...
export default function ProjectEditPage() {
  ...
  const mutation = useMutation({ mutationFn: async () => { await updateMyProject(project!.id, { slug: effectiveSlug, title: draft.title, summary: draft.summary, demo_url: draft.demo, repo_url: draft.repo, visibility: draft.visibility, hackathon_track_ids: draft.tracks, hackathon_sector_ids: draft.sectors, hackathon_tech_stack_ids: draft.tech }); }, onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["projects"] }); toast.success(t("projects.form.saved")); navigate(`/projects/${effectiveSlug}`); }, onError: (error) => toast.error(error instanceof Error ? error.message : t("projects.form.saveFailed")) });
