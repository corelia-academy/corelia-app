...
export default function ProjectEditPage() {
  ...
  const contestQuery = useQuery({ placeholderData: (previous, query) => query?.queryKey[1] === project?.source_id ? previous : undefined, queryKey: ["hackathons", project?.source_id, "project-edit", locale], queryFn: () => getContest(project!.source_id!, locale), enabled: Boolean(project?.source_id && hackathon) });
