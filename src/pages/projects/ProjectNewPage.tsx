...
export default function ProjectNewPage() {
  ...
  if (hackathonSlug && (contestQuery.isPending || (contest && contextQuery.isPending))) return <div className="container-app py-16" role="status">{t("projects.loading")}</div>;
  if (contestQuery.isError || contextQuery.isError) return <div className="container-app py-16" role="alert"><p>{t("projects.errorDescription")}</p><Button onClick={() => { void contestQuery.refetch(); void contextQuery.refetch(); }}>{t("projects.retry")}</Button></div>;
