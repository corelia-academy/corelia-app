...
export default function FeedPage() {
  ...
  return <div className="container-app py-6 sm:py-8"><div className="mx-auto max-w-6xl">
    ...
    <Tabs.Root value={mode} onValueChange={(value) => { if (value === "explore" || value === "following") setMode(value); }}>
      ...
      <Tabs.Panel value={mode} className="pt-3">
        <p className="text-sm text-foreground-muted">{t(following ? "milestones.followingDescription" : "milestones.exploreDescription")}</p>
        {following && <Button type="button" variant="ghost" className="mt-2 px-0" onClick={() => setMode("explore")}>{t("milestones.findPeople")}</Button>}
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]"><main className="min-w-0 space-y-3">
    ...
    </main><aside className="hidden space-y-5 lg:block"><FeedSuggestedPeople userId={userId} /><FeedFollowingPanel userId={userId} /></aside></div>
      </Tabs.Panel>
    </Tabs.Root>
  </div></div>;
}
