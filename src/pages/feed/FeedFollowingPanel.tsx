...
function profileName(row: FeedSuggestedProfile): string {
...
export function FeedFollowingPanel({ userId }: { userId: string }) {
  ...
  const suggest = query.isSuccess && rows.length === 0;
  const suggestions = useQuery(feedSuggestionsQueryOptions(userId, suggest));
  const preview = rows.slice(0, 6);
  const hasMore = rows.length > 6 || query.hasNextPage;
  const title = t(suggest ? "milestones.suggestedTitle" : "milestones.followedTitle");
  const previewContent = query.isPending ? <div className="mt-3 space-y-2"><Skeleton className="h-12" /><Skeleton className="h-12" /></div>
    ...
    : suggest ? suggestions.isPending ? <div className="mt-3 space-y-2"><Skeleton className="h-12" /><Skeleton className="h-12" /></div>
    ...
    : <><div className="mt-2 space-y-1">{preview.map((row) => <FollowingPerson key={row.id} row={row} />)}</div>{hasMore && <Button type="button" size="sm" variant="ghost" className="mt-2 w-full" onClick={() => setOpen(true)}>{t("milestones.followedAll")}</Button>}</>;
