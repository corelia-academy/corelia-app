...
function SourcesSection({
  ...
  return <>
    ...
    <section><h2 className="text-heading-medium font-display">{t("admin.source.feedTitle")}</h2><p className="mt-1 text-sm text-foreground-muted">{t("admin.source.feedDescription")}</p><div className="mt-3">{renderTable(directSources, false)}</div></section>
    <section className="mt-8"><h2 className="text-heading-medium font-display">{t("admin.source.atsTitle")}</h2><p className="mt-1 text-sm text-foreground-muted">{t("admin.source.atsDescription")}</p><div className="mt-3">{renderTable(atsSources, true)}</div></section>
  </>;
}
...
function CompaniesSection({ companies, sources, busy, onRun, onSaved }: { companies: JobCompanyAdmin[]; sources: JobSourceAdmin[]; busy: boolean; onRun: (id: string, mode?: "discovery" | "revalidation") => void; onSaved: () => Promise<void> }) {
  ...
  return <>
    <div className="mb-5 rounded-xl border border-border-subtle bg-surface-raised/40 p-4"><h2 className="text-heading-small font-display">{t("admin.company.title")}</h2><p className="mt-1 text-sm leading-6 text-foreground-muted">{t("admin.company.description")}</p></div>
