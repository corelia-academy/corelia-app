...
function ReviewSection({ jobs, busy, onReview }: { jobs: Array<{ id: string; title: string; company_name: string; summary: string | null; job_type: "tech" | "non_tech"; primary_role: string | null; domains: string[]; required_skills: string[]; classification_confidence: number | null; review_reason: string | null }>; busy: boolean; onReview: (id: string, status: "active" | "rejected") => void }) {
  const { t } = useTranslation("jobs");
  return jobs.length ? <div className="grid gap-4 lg:grid-cols-2">{jobs.map((job) => <Card key={job.id}><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">{job.title}</h2><p className="mt-1 text-sm text-foreground-muted">{job.company_name}</p></div><span className="rounded-full bg-amber-500/10 px-2 py-1 text-xs text-amber-700 dark:text-amber-300">{Math.round((job.classification_confidence ?? 0) * 100)}%</span></div>{job.summary ? <p className="mt-4 text-sm leading-6 text-foreground-muted">{job.summary}</p> : null}<div className="mt-3 flex flex-wrap gap-1.5">{[job.job_type, job.primary_role, ...job.domains, ...job.required_skills].filter(Boolean).slice(0, 8).map((value) => <span key={value} className="rounded-full bg-surface-raised px-2 py-1 text-xs">{value === "tech" || value === "non_tech" ? t(`values.${value}`) : humanizeJobSlug(value)}</span>)}</div><p className="mt-4 text-xs text-foreground-subtle">{t("admin.review.reason")}: {job.review_reason || "—"}</p><div className="mt-4 flex gap-2"><Button type="button" disabled={busy} onClick={() => onReview(job.id, "active")}><Check className="size-4" aria-hidden />{t("admin.actions.publish")}</Button><Button type="button" variant="destructive" disabled={busy} onClick={() => onReview(job.id, "rejected")}><X className="size-4" aria-hidden />{t("admin.actions.reject")}</Button></div></CardContent></Card>)}</div> : <p className="rounded-xl border border-border-subtle bg-surface-base p-10 text-center text-sm text-foreground-muted">{t("admin.review.empty")}</p>;
}
