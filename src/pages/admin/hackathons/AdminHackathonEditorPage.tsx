...
function notifyAwardDeliveryResults(
  res: { emails_sent_count?: number; failures_count?: number; failures?: Array<{ reason: string; is_retryable: boolean }> } | null,
  t: TFunction<"admin">,
) {
  if (!res) return;
  const failures = res.failures ?? [];
  const hasNotConfigured = failures.some((f) => f.reason === "email_not_configured" || f.reason === "provider_config_error");
  const hasIndeterminate = failures.some((f) => f.reason.startsWith("indeterminate") || f.reason === "dispatch_window_expired");
  const hasPermanent = failures.some(
    (f) =>
      !f.is_retryable &&
      !f.reason.startsWith("indeterminate") &&
      f.reason !== "dispatch_window_expired" &&
      f.reason !== "email_not_configured" &&
      f.reason !== "provider_config_error",
  );
  ...
  if (hasNotConfigured) {
    toast.warning(t("hackathons.editor.awardsNotifyNotConfigured"));
  }
  if (hasIndeterminate) {
    toast.error(t("hackathons.editor.awardsNotifyIndeterminateError"));
  }
  if (hasPermanent) {
    toast.error(t("hackathons.editor.awardsNotifyPermanentError"));
  }
  if (hasRetryable) {
    toast.warning(t("hackathons.editor.awardsNotifyRetryableError"));
  }

  if (res.emails_sent_count && res.emails_sent_count > 0) {
    toast.success(t("hackathons.editor.awardsNotified", { count: res.emails_sent_count }));
  }
}
...
export default function AdminHackathonEditorPage() {
  ...
  const { hasNextPage: hasMoreProjects, isFetchingNextPage: isFetchingMoreProjects, fetchNextPage: fetchNextProjectsPage } = projectsQuery;
  useEffect(() => {
    if (activeSection === "projects" && hasMoreProjects && !isFetchingMoreProjects) {
      void fetchNextProjectsPage();
    }
  }, [activeSection, hasMoreProjects, isFetchingMoreProjects, fetchNextProjectsPage]);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (draft.winner_awards.length > 0) {
        try {
          const awardRes = await notifyHackathonWinnerAwards(
            contest.id,
          ...
          notifyAwardDeliveryResults(awardRes, t);
        } catch (awardErr) {
  ...
  const notifyAwardsMutation = useMutation({
    mutationFn: async () => {
      if (!id || draft.winner_awards.length === 0) return null;
      return await notifyHackathonWinnerAwards(
        id,
        draft.winner_awards.map((award) => ({
          project_id: award.project_id,
          label: award.label,
    ...
    onSuccess: (res) => {
      notifyAwardDeliveryResults(res, t);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("hackathons.editor.awardsNotifyFailed"));
    },
  });
