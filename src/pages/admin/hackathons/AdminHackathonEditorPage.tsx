...
import { AlertTriangle, ArrowLeft, Camera, Check, ImageIcon, Plus, Save, Trash2, Trophy } from "lucide-react";
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <main className="space-y-6">
          <Section id="overview" title={t("hackathons.editor.sections.overview")} onSave={save} saving={saveMutation.isPending}>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-surface-raised/30">
              <div className="relative flex min-h-48 items-center justify-center overflow-hidden bg-surface-overlay sm:aspect-[21/9] sm:max-h-80">
                {bannerPreviewUrl || (!bannerRemoved && draft.cover_image_url) ? <img src={bannerPreviewUrl ?? draft.cover_image_url} alt={t("hackathons.editor.fields.bannerPreviewAlt")} className="absolute inset-0 size-full object-cover" /> : <div className="flex flex-col items-center gap-2 px-6 text-center text-foreground-muted"><ImageIcon className="size-10" /><p className="text-sm font-medium">{t("hackathons.editor.fields.bannerEmpty")}</p><p className="text-xs">{t("hackathons.editor.fields.bannerHint")}</p></div>}
                <div className="absolute bottom-4 right-4 flex items-center gap-2">
                  {bannerPreviewUrl || (!bannerRemoved && draft.cover_image_url) ? <Button type="button" variant="secondary" size="icon-lg" className="min-h-11 min-w-11 border border-border bg-background/90 shadow-md backdrop-blur" aria-label={t("hackathons.editor.removeImage")} onClick={() => { setBannerFile(null); setBannerPreviewUrl(null); setBannerRemoved(true); setDirty(true); }}><Trash2 className="size-4" /></Button> : null}
                  <label htmlFor="hackathon-banner-upload" className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-md border border-border bg-background/90 px-4 text-sm font-medium text-foreground shadow-md backdrop-blur transition-colors hover:bg-background focus-within:ring-2 focus-within:ring-primary/40"><Camera className="size-4" />{bannerPreviewUrl || (!bannerRemoved && draft.cover_image_url) ? t("hackathons.editor.fields.changeBanner") : t("hackathons.editor.fields.uploadBanner")}</label>
                  <Input id="hackathon-banner-upload" className="sr-only" type="file" accept="image/*" aria-label={t("hackathons.editor.fields.banner")} onChange={(event) => { const file = event.target.files?.[0] ?? null; setBannerFile(file); setBannerPreviewUrl(file ? URL.createObjectURL(file) : null); setBannerRemoved(false); if (file) setDirty(true); event.target.value = ""; }} />
                </div>
              </div>
              <div className="relative px-5 pb-5 pt-20 sm:px-6 sm:pt-5">
                <div className="absolute -top-14 left-5 size-28 rounded-full border-4 border-surface-base bg-surface-overlay shadow-lg sm:left-6">
                  <div className="flex size-full items-center justify-center overflow-hidden rounded-full bg-surface-overlay text-foreground-muted">
                    {hostLogoPreviewUrl || (!hostLogoRemoved && draft.host_logo_url) ? <img src={hostLogoPreviewUrl ?? draft.host_logo_url} alt={t("hackathons.editor.fields.hostLogoPreviewAlt")} className="size-full bg-white object-contain p-2" /> : <ImageIcon className="size-8" />}
                  </div>
                  {hostLogoPreviewUrl || (!hostLogoRemoved && draft.host_logo_url) ? <Button type="button" variant="secondary" size="icon-lg" className="absolute -left-1 bottom-0 size-11 rounded-full border border-border bg-background shadow" aria-label={t("hackathons.editor.removeImage")} onClick={() => { setHostLogoFile(null); setHostLogoPreviewUrl(null); setHostLogoRemoved(true); setDirty(true); }}><Trash2 className="size-4" /></Button> : null}
                  <label htmlFor="hackathon-host-logo-upload" className="absolute -right-1 bottom-0 flex size-11 cursor-pointer items-center justify-center rounded-full border border-border bg-background text-foreground shadow transition-colors hover:bg-surface-raised focus-within:ring-2 focus-within:ring-primary/40" title={t("hackathons.editor.fields.changeHostLogo")}><Camera className="size-4" /></label>
                  <Input id="hackathon-host-logo-upload" className="sr-only" type="file" accept="image/*" aria-label={t("hackathons.editor.fields.hostLogo")} onChange={(event) => { const file = event.target.files?.[0] ?? null; setHostLogoFile(file); setHostLogoPreviewUrl(file ? URL.createObjectURL(file) : null); setHostLogoRemoved(false); if (file) setDirty(true); event.target.value = ""; }} />
                </div>
                <div className="grid gap-4 sm:ml-32 sm:grid-cols-2"><label className="text-sm font-medium">Host<Input className="mt-2" value={draft.host_name} onChange={(event) => change({ host_name: event.target.value })} /></label><label className="text-sm font-medium">Host website<Input className="mt-2" type="url" value={draft.host_website_url} onChange={(event) => change({ host_website_url: event.target.value })} /></label></div>
              </div>
            </div>
