...
export function ContestDetailOverviewBlocks({
  ...
  const showDescriptionCard =
    Boolean(contest.description?.trim()) &&
    ((!isManageView) || (isManageView && activeManageSection === "overview"));

  return (
    <>
      {showDescriptionCard ? (
        <Card
          id={!isManageView ? "about" : undefined}
          className={cn(!isManageView && "scroll-mt-36")}
        >
