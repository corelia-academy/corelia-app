...
import { BookmarkX, BriefcaseBusiness, EyeOff } from "lucide-react";
...
export function UserJobsPage({ mode }: { mode: "saved" | "applied" | "hidden" }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      if (variables.patch.hidden === false) toast.success(t("messages.unhidden"));
    },
    onError: () => toast.error(t("messages.updateFailed")),
  });
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8"><JobsNav /><header className="mt-6"><h1 className="text-heading-medium font-display">{t(`user.${mode}.title`)}</h1><p className="mt-2 text-sm text-foreground-muted">{t(`user.${mode}.description`)}</p></header>{query.isPending ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 3 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-2xl bg-surface-raised" />)}</div> : query.data?.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{query.data.map(({ job, state }) => <JobCard key={job.id} job={job} state={state} busy={mutation.isPending && mutation.variables?.jobId === job.id} onToggleSaved={() => mutation.mutate({ jobId: job.id, patch: { saved: !state.saved } })} onToggleApplied={() => mutation.mutate({ jobId: job.id, patch: { applied: !state.applied } })} onToggleHidden={() => mutation.mutate({ jobId: job.id, patch: { hidden: !state.hidden } })} />)}</div> : <div className="mt-6 rounded-xl border border-border-subtle bg-surface-base p-12 text-center">{mode === "saved" ? <BookmarkX className="mx-auto size-8 text-foreground-subtle" aria-hidden /> : mode === "hidden" ? <EyeOff className="mx-auto size-8 text-foreground-subtle" aria-hidden /> : <BriefcaseBusiness className="mx-auto size-8 text-foreground-subtle" aria-hidden />}<h2 className="mt-3 text-heading-small font-display">{t(`user.${mode}.empty`)}</h2></div>}</div>;
}
...
export function HiddenJobsPage() { return <UserJobsPage mode="hidden" />; }
