...
export default function JobDetailPage() {
  ...
  const canIndex = metaJob?.job_sources?.allow_seo_indexing === true;
  usePageMeta({
    title: metaJob ? `${metaJob.title} at ${metaJob.company_name}` : undefined,
    description: metaJob?.summary ?? undefined,
    url: pageUrl,
    canonicalUrl: pageUrl,
    robots: metaJob ? canIndex ? "index,follow" : "noindex,nofollow" : "noindex,follow",
  });
  ...
  if (jobQuery.isError) return <div role="alert" className="mx-auto max-w-3xl p-8 text-center"><h1>{t("messages.loadFailed")}</h1><Button className="mt-4" variant="outline" onClick={() => void jobQuery.refetch()}>{t("retry")}</Button></div>;
  if (!jobQuery.data) return <div className="mx-auto max-w-3xl p-8 text-center"><h1 className="text-heading-medium font-display">{t("detail.notFound")}</h1><Button render={<Link to="/jobs" />} variant="outline" className="mt-4">{t("detail.back")}</Button></div>;
  ...
  const structuredData = {
    ...
    ...(job.remote_type !== "remote" && job.location_text ? {
      jobLocation: { "@type": "Place", address: { "@type": "PostalAddress", addressLocality: job.location_text } },
    } : {}),
    ...(job.country_codes.length || job.regions.length ? {
      applicantLocationRequirements: [
        ...job.country_codes.map((name) => ({ "@type": "Country", name })),
        ...job.regions.map((name) => ({ "@type": "AdministrativeArea", name })),
      ],
    } : {}),
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5">
          <Card><CardContent className="p-6 sm:p-8"><div className="flex items-start gap-4">{job.company_logo_url ? <div className="flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border-subtle bg-surface-raised"><img src={job.company_logo_url} alt="" className="size-full object-contain p-1.5" /></div> : null}<div><p className="text-sm font-medium text-primary">{job.company_name}</p><h1 className="mt-1 text-heading-medium font-display">{job.title}</h1><div className="mt-3 flex flex-wrap gap-3 text-sm text-foreground-muted"><span className="inline-flex items-center gap-1"><MapPin className="size-4" aria-hidden />{job.location_text || humanizeJobSlug(job.remote_type)}</span><span className="inline-flex items-center gap-1"><BriefcaseBusiness className="size-4" aria-hidden />{humanizeJobSlug(job.employment_type)}</span><span>{formatJobDate(job.posted_at ?? job.first_seen_at, i18n.language)}</span></div></div></div>{job.summary ? <p className="mt-6 text-base leading-7 text-foreground-muted">{job.summary}</p> : null}</CardContent></Card>
          <Card><CardContent className="p-6 sm:p-8"><h2 className="text-heading-medium font-display">{t("detail.about")}</h2><div className="mt-4 max-w-[720px] whitespace-pre-wrap text-sm leading-7 text-foreground-muted">{description || t("detail.noDescription")}</div></CardContent></Card>
          <div className="grid gap-5 md:grid-cols-2">
            <Card><CardContent className="p-6"><h2 className="text-heading-small font-display">{t("detail.requiredSkills")}</h2><div className="mt-3 flex flex-wrap gap-2">{job.required_skills.length ? job.required_skills.map((skill) => <Link key={skill} to={`/jobs?skill=${encodeURIComponent(skill)}`} className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{humanizeJobSlug(skill)}</Link>) : <span className="text-sm text-foreground-muted">—</span>}</div></CardContent></Card>
            <Card><CardContent className="p-6"><h2 className="text-heading-small font-display">{t("detail.preferredSkills")}</h2><div className="mt-3 flex flex-wrap gap-2">{job.preferred_skills.length ? job.preferred_skills.map((skill) => <span key={skill} className="rounded-full bg-surface-raised px-2.5 py-1 text-xs text-foreground-muted">{humanizeJobSlug(skill)}</span>) : <span className="text-sm text-foreground-muted">—</span>}</div></CardContent></Card>
          </div>
        </div>
        <aside className="space-y-4 lg:sticky lg:top-[calc(var(--app-header-height)+1rem)] lg:self-start">
          ...
          <Card><CardContent className="space-y-3 p-5 text-sm"><h2 className="text-heading-small font-display">{t("detail.overview")}</h2><div><span className="text-foreground-muted">{t("detail.salary")}</span><p className="mt-0.5 font-medium">{salary ?? t("card.salaryNotShown")}</p></div><div><span className="text-foreground-muted">{t("detail.jobType")}</span><p className="mt-0.5 font-medium">{t(`values.${job.job_type}`)}</p></div><div><span className="text-foreground-muted">{t("detail.seniority")}</span><p className="mt-0.5 font-medium">{humanizeJobSlug(job.seniority)}</p></div><div><span className="text-foreground-muted">{t("detail.role")}</span><p className="mt-0.5 font-medium">{humanizeJobSlug(job.primary_role)}</p></div><div><span className="text-foreground-muted">{t("detail.source")}</span><p className="mt-0.5 font-medium">{job.job_sources?.name ?? "—"}</p>{job.job_sources?.attribution_required ? <a href={job.source_url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-foreground-muted underline-offset-4 hover:text-foreground hover:underline">{job.job_sources.attribution_text || t("detail.sourceAttribution", { source: job.job_sources.name })}<ExternalLink className="size-3" aria-hidden /></a> : null}</div></CardContent></Card>
        </aside>
      </div>
    </div>
  );
