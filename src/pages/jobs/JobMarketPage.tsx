...
function ActivityChart({ rows }: { rows: MarketDailyStat[] }) {
  ...
  if (!visible.length) return <p className="py-8 text-sm text-foreground-muted">{t("market.empty.description")}</p>;
  const max = Math.max(1, ...visible.map((row) => Math.max(row.new_jobs, row.expired_jobs)));
  return <div><p className="mb-3 text-xs text-foreground-muted">{t("market.chartScale", { count: max })}</p><div className="flex h-40 items-end gap-1" role="img" aria-label={t("market.activityDescription")}>{visible.map((row) => <div key={row.date} className="flex h-full min-w-0 flex-1 items-end gap-px" title={`${row.date}: +${row.new_jobs} / -${row.expired_jobs}`}><span className="min-h-px flex-1 rounded-t-sm bg-primary" style={{ height: `${Math.max(2, row.new_jobs / max * 100)}%` }} /><span className="min-h-px flex-1 rounded-t-sm bg-destructive/60" style={{ height: `${Math.max(2, row.expired_jobs / max * 100)}%` }} /></div>)}</div><div className="mt-2 flex justify-between text-xs text-foreground-muted"><span>{visible[0]?.date}</span><span>{visible.at(-1)?.date}</span></div><div className="mt-3 flex gap-4 text-xs text-foreground-muted"><span className="inline-flex items-center gap-1.5"><i className="size-2 rounded-sm bg-primary" />{t("market.newJobs")}</span><span className="inline-flex items-center gap-1.5"><i className="size-2 rounded-sm bg-destructive/60" />{t("market.expiredJobs")}</span></div></div>;
}
...
export default function JobMarketPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8"><JobsNav /><header className="mt-6"><div className="flex items-center gap-2 text-sm font-semibold text-primary"><BarChart3 className="size-4" aria-hidden />{t("market.eyebrow")}</div><h1 className="mt-2 text-heading-medium font-display">{t("market.title")}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-foreground-muted">{t("market.subtitle")}</p></header>{query.isPending ? <div className="mt-6 h-80 animate-pulse rounded-2xl bg-surface-raised" /> : query.isError ? <div role="alert" className="mt-6 rounded-2xl border border-destructive/30 p-6"><p>{t("messages.loadFailed")}</p><Button className="mt-4" variant="outline" onClick={() => void query.refetch()}>{t("retry")}</Button></div> : !latest ? <Card className="mt-6"><CardContent className="p-12 text-center"><BarChart3 className="mx-auto size-8 text-foreground-subtle" aria-hidden /><h2 className="mt-3 text-heading-small font-display">{t("market.empty.title")}</h2><p className="mt-1 text-sm text-foreground-muted">{t("market.empty.description")}</p></CardContent></Card> : <><div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(([label, value, Icon, href]) => <Link key={label} to={href}><Card className="h-full transition-colors hover:bg-surface-raised"><CardContent className="p-5"><Icon className="size-5 text-primary" aria-hidden /><p className="mt-4 text-2xl font-bold">{typeof value === "number" ? value.toLocaleString() : value}</p><p className="mt-1 text-sm text-foreground-muted">{label}</p>{label === t("market.cards.remote") ? <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-surface-raised"><div className="h-full rounded-full bg-primary" style={{ width: `${remoteShare}%` }} /></div> : null}</CardContent></Card></Link>)}</div><div className="mt-5 grid gap-5 lg:grid-cols-2"><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.activity")}</h2><div className="mt-4"><ActivityChart rows={snapshot.daily} /></div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.seniorityMix")}</h2><div className="mt-3"><StatList rows={snapshot.seniorities} field="seniority" total={latest.active_jobs} /></div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.topRoles")}</h2><div className="mt-3"><StatList rows={snapshot.roles} field="role" total={latest.active_jobs} /></div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.roleGrowth")}</h2><div className="mt-3"><GrowthList rows={snapshot.roleHistory} field="role" /></div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.topSkills")}</h2><div className="mt-3"><StatList rows={snapshot.skills} field="skill" total={latest.active_jobs} /></div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.skillGrowth")}</h2><div className="mt-3"><GrowthList rows={snapshot.skillHistory} field="skill" /></div></CardContent></Card><Card className="lg:col-span-2"><CardContent className="p-5"><h2 className="text-heading-small font-display">{t("market.topDomains")}</h2><div className="mt-3 grid gap-x-5 md:grid-cols-2"><StatList rows={snapshot.domains} field="domain" total={latest.active_jobs} /></div></CardContent></Card></div><p className="mt-5 text-xs text-foreground-subtle">{t("market.disclaimer")}</p></>}</div>;
}
