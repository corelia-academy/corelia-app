...
function rowToProfile(row: Record<string, unknown>): Profile {
  return {
    ...
    is_creator: (row.is_creator as boolean | null) ?? false,
    creator_first_published_at: (row.creator_first_published_at as string | null) ?? null,
    trust_score: Number(row.trust_score ?? 0),
    spam_strikes: Number(row.spam_strikes ?? 0),
    publishing_blocked_until: (row.publishing_blocked_until as string | null) ?? null,
...
async function _fetchProfileForUser(user: User): Promise<Profile | null> {
  ...
  if (!row) {
    const insert: ProfileInsert & { created_at: string; updated_at: string } = {
      ...
      is_creator: false,
      creator_first_published_at: null,
