...
Đã triển khai cả bốn entity. `worker/og.ts` giữ public route và HTML rewrite; `corelia-api/og/` đọc DTO public, tính revision từ nội dung card, tải media nội bộ và render PNG. Course dùng slug canonical, `/courses/:id` là alias. Profile chỉ dùng avatar từ Storage nội bộ; avatar URL ngoài chuyển sang monogram. Ngày hackathon luôn định dạng theo `Asia/Ho_Chi_Minh`.
