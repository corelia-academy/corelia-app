...
- Thanh toán: chỉ dùng **chế độ test** (Stripe/SePay) — không thẻ thật. Ngữ cảnh model access & revenue: [COURSE_OWNERSHIP_REVENUE_SEPAY.md](../COURSE_OWNERSHIP_REVENUE_SEPAY.md), [COURSE_ACCESS_MODELS.md](../COURSE_ACCESS_MODELS.md).
...
4. Điều hướng `/checkout/course/:courseId` — form/step không crash.
...
9. **Không có quyền**: truy cập URL learn trực tiếp — kỳ vọng bị chặn hoặc redirect phù hợp (ghi nhận hành vi hiện tại là pass/fail theo spec).

### Redirect legacy

10. `/cohorts` và `/cohorts/:id` redirect về `/courses` (không 404).

## Kết quả mong đợi

- Luồng xem → (tuỳ quyền) checkout → học hoạt động trên staging test payment.
