...
3. Nếu team dùng short URL `/<handle>/...`: thử path có seed — redirect đúng hoặc landing đúng ([App.tsx](../../src/App.tsx) route `:handle/*`). Tránh handle trùng reserved route (`hackathons`, `career`, `courses`, …).
